...
//    double textScaleFactor = map['textScaleFactor'];
    ...
    if (map.containsKey("textSpan")) {
      textSpan = textSpanParser.parse(map['textSpan'], buildContext, listener);
    }
...
class SelectableTextSpanParser {
  TextSpan parse(Map<String, dynamic> map, BuildContext buildContext,
      ClickListener listener) {
    ...
    if (map.containsKey('children')) {
      parseChildren(textSpan, map['children'], buildContext, listener);
    }
  ...
  void parseChildren(TextSpan textSpan, List<dynamic> childrenSpan,
    ...
    for (var childmap in childrenSpan) {
      textSpan.children.add(parse(childmap, buildContext, listener));
    }
  }
